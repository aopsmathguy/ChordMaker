...
function keyDetect(chordOccurences) {
    ...
    for (const chord in chordOccurences) {
        ...
        chordOccurencesWithoutVariants[cleanedChord] = chordOccurences[chord] + (chordOccurencesWithoutVariants[cleanedChord] || 0);
    }
    ...
    for (let i = 0; i < 12; i++){
        ...
        for (const chord in chordOccurencesWithoutVariants) {
            ...
            score += (mostCommonCMajorChords[newChord] || 0) * chordOccurencesWithoutVariants[chord];
        }
        if (score > bestScore) {
            bestScore = score;
            bestKey = i;
...
var SongTab = class {
    ...
    toText(columns = 2) {
        ...
        let heightLeft = sectionsHeights.reduce((sum, h) => sum + h);
        const allocations = [];
        let nextSection = 0;
        for (let currColumn = 0; currColumn < columns; currColumn++) {
            const goal = heightLeft / (columns - currColumn);
            let currColHeight = 0;
            allocations.push([]);
            const addTo = allocations[allocations.length - 1];
            while (currColHeight + sectionsHeights[nextSection]/2 < goal) {
                addTo.push(nextSection);
                currColHeight += sectionsHeights[nextSection];
                heightLeft -= sectionsHeights[nextSection];
                nextSection++;
            }
        }
        // Create the text for each column
        const columnWidths = allocations.map((sectionIndices, col) => {
            return sectionIndices.reduce((max, i) => Math.max(max, this.sections[i].getWidth()), 0);
        });
        const columnTexts = allocations.map((sectionIndices, col) => {
            ...
            for (let j = 0; j < sectionIndices.length; j++) {
                const sectionTexts = this.sections[sectionIndices[j]].toTexts(columnWidth);
        ...
        let textLines = [`[b]${this.title}[/b] - ${this.artist}`,`Key: [ch]${this.key}[/ch]`, '_'.repeat(columnWidths.reduce((sum, w) => sum + w, columns - 1))];
        const maxColumnHeight = columnTexts.reduce((max, col) => Math.max(max, col.length), 0);
        ...
        for (let i = 0; i < this.sections.length; i++) {
            ...
            for (let j = 0; j < section.lines.length; j++) {
                ...
                for (let k = 0; k < line.chordLyricPairs.length; k++) {
                    ...
                    if (chord.trim() !== "") {
                        chordOccurences[chord] = (chordOccurences[chord] || 0) + 1;
                    }
                }
            }
        }
...
var SongTabSection  = class{
    ...
    getWidth() {
        return Math.max(this.header.length, this.lines.reduce((max, line) => {
...
var SongTabLine  = class{
    ...
    transpose(d){
        for (let i = 0; i < this.chordLyricPairs.length; i++) {
            this.chordLyricPairs[i].chord = transposeChord(this.chordLyricPairs[i].chord, d);
        }
    }
    getWidth() {
    ...
    toTexts(width) {
        ...
        for (let i = 0; i < this.chordLyricPairs.length; i++) {
            ...
            const chordText = `[ch]${chord}[/ch]` + " ".repeat(usedWidth - chord.length);
        ...
        */
        ...
        for (let i = 0; i < chordLyricPairs.length; i++) {
            ...
            if (lyrics === "") {
                ...
                .filter(index => index !== -1).filter(index => index >= chordLength);
                breakpoints.push(...segmentBreaks.map(breakpoint => ([i, breakpoint, breakpoint + position])));
            }
        ...
        for (let i = 0; i < breakpoints.length; i++) {
            ...
            const [lastBreakSegmentIdx, lastBreakpoint, lastBreakpointPosition] = breakpoints[lastBreakAccepted];
            if (position - lastBreakpointPosition > maxLineWidth) {
                // add the line from lastBreakAccepted to i - 1
                const newLine = new SongTabLine();
                const [prevPairIndex, prevBreakpoint, prevPosition] = breakpoints[i - 1] || [0, 0, 0];
                if (lastBreakSegmentIdx == prevPairIndex) {
                    const [chord, lyrics] = chordLyricPairs[prevPairIndex];
                    newLine.addPair([lastBreakpoint == 0 ? chord : "", lyrics.slice(lastBreakpoint, prevBreakpoint)]);
                } else {
                    let [chord, lyrics] = chordLyricPairs[lastBreakSegmentIdx];
                    newLine.addPair([lastBreakpoint == 0 ? chord : "", lyrics.slice(lastBreakpoint)]);
                    for (let j = lastBreakSegmentIdx + 1; j < prevPairIndex; j++) {
        ...
        //add the last line
        const newLine = new SongTabLine();
        const [lastBreakSegmentIdx, lastBreakpoint] = breakpoints[lastBreakAccepted];
        let [chord, lyrics] = chordLyricPairs[lastBreakSegmentIdx];
        newLine.addPair([lastBreakpoint == 0 ? chord : "", lyrics.slice(lastBreakpoint)]);
...
function applyTransposeUp() {
    const chords = document.querySelectorAll('td.chord, .matchedChordContainer');
    chords.forEach(c => {
...
function popUpPage(data) {
    console.log(data);
    const { columns, maxWidth, theme } = data || { columns: 2, maxWidth: 50, theme : [[255, 255, 255], [0, 0, 0], [255, 0, 0]] };
    ...
    if (document.querySelector('.song-part-pages') !== null) {
        ...
        const [artist, title] = [header.querySelector('h4'), header.querySelector('h1')].map(c => c.innerText.trim());
        ...
        const sections = [...sectionElems].map(s => {
            ...
            const lines = [...s.querySelectorAll('table')].map(l => {
                const chords = [...l.querySelectorAll('td.chord')].map(c => c.innerText.trim() + ' ');
                const lyrics = [...l.querySelectorAll('td.lyrics')].map(c => 
                    c.innerText.replace(/\s\s/g, ' ')
                );
                const chordLyricPairs = chords.map((c, i) => [c, lyrics[i]]).filter(p => p[0].trim() !== '' || p[1].trim() !== '');
        ...
        const title = songDetails.querySelector('h1.t-song-details__marquee__headline').innerText.trim();
        const artist = songDetails.querySelector('p.large').innerText.trim();
        const lineElems = document.querySelector('.chord-pro-disp').querySelectorAll('.chord-pro-line');
        ...
        for (let i = 0; i < lineElems.length; i++) {
            ...
            const allChords = [...lineElem.querySelectorAll('.chord-pro-note')].map(c => c.innerText).join('').trim();
            const wholeLineLyrics = [...lineElem.querySelectorAll('.chord-pro-lyric')].map(c => c.innerText).join('').trim();
            if (allChords == '' && regex.test(wholeLineLyrics)) {
                if (currentSection.lines.length > 0 || currentSection.header !== "") {
                    sections.push(currentSection);
                }
                currentSection = new SongTabSection({ header: wholeLineLyrics });
            } else{
                const newLine = new SongTabLine();
        ...
        const lines = mainParent.querySelector('pre').innerText.split('\n').map(line => line.replace(/\r/g, ''));
        const classifications = lines.map(function(line) {
            ...
            const chordRegex = /^[A-G][#b]?((m|maj|min|dim|aug|sus\d?|add\d?|maj7|m7|dim7|aug7|7|9|11|13|2|4|6)?)?(\/[A-G][#b]?)?$/i;
            ...
            tokens.forEach(token => {
                ...
                if (chordRegex.test(cleanToken) || noChordRegex.test(cleanToken)) {
                    chordCount++;
                }
                totalTokens++;
            });
        ...
        for (let i = 0; i < lines.length - 1; i++) {
            ...
            if (currClass === 'Section Header') {
                ...
                currentSection = new SongTabSection({ header: currLine.replace(/[\[\]]/g, '') });
                ...
                if (i == lines.length - 1 || classifications[i + 1] === 'Chord Line' || classifications[i + 1] === 'Empty') {
                    ...
                    while ((match = regex.exec(currLine)) !== null) {
                        ...
                        const separators = " ".repeat(match[2].length + match[1].length);          // The separators after the token
                    ...
                    if (leadingSpacesMatch) {
                        const leadingSpaces = leadingSpacesMatch[0].length; // Extract the leading spaces
                        newLine.addPair(["", nextLine.slice(0, leadingSpaces).padEnd(leadingSpaces, " ")]);
                        index = leadingSpaces;
                    }
                    let match;
                    // Iterate over all matches
                    while ((match = regex.exec(currLine)) !== null) {
                        ...
                        const separators = nextLine.slice(index, endIndex).padEnd(lengthLyrics, " ");
                        index = endIndex;
                        newLine.addPair([token, separators]);
                    }
    ...
    const [backgroundColor, textColor, chordColor] = theme || [[255, 255, 255], [0, 0, 0], [255, 0, 0]];
    ...
    pdf.textWithLink(window.location.href, margins + generatedByText.length * footnoteWidth, pdfHeight - margins, { url: window.location.href });
